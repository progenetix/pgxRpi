...
* `type`: A string specifying output data type. Only "g_variants" is used in this tutorial. 
* `output`: A string specifying output file format. When the parameter `type` is "g_variants",
available options are NULL, "pgxseg" ,"pgxmatrix", "cnvfraction" or "seg".
...
The value is the fraction of how much the binned interval overlaps with one or more CNVs of the given type (DUP/DEL). For example, if the value in the second row, the first column is 0.2, it means that one or more duplication events overlapped with 20% of the genomic bin located in chromosome 1: 400000-1400000 in the first analysis profile.
...
```{r}
cnv_fraction <- pgxLoader(type="g_variants", output="cnvfraction", filters = "NCIT:C2948")
```
