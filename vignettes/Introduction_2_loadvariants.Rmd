...
* `type` A string specifying output data type. Available options are "b" (biosample), "i" (individual), "v" (variant) or "f" (frequency). 
...
type, output, filters, individual_id, biosample_id, codematches, skip, limit

## Across genomic bins 

```{r}
cnv_matrix <- pgxLoader(type="v", output="pgxmatrix", filters = "NCIT:C2948")
```
...
```{r}
cnv_covergae <- pgxLoader(type="v", output="coverage", filters = "NCIT:C2948")
```
...
```{r}
cnv_covergae_2 <- pgxLoader(type="v", output="coverage", filters = "NCIT:C2948",
                            codematches = TRUE)
...
```{r}
cnv_matrix_2 <- pgxLoader(type="v", output="pgxmatrix", 
                          filters = "NCIT:C2948",
                          skip = 0, limit=10)
...
```{r}
cnv_ind_matrix <- pgxLoader(type="v", output="pgxmatrix", 
                          biosample_id = "pgxbs-kftva604",
                          individual_id = "pgxind-kftx5g4t")

cnv_ind_cov <- pgxLoader(type="v", output="coverage", 
                          biosample_id = "pgxbs-kftva604",
                          individual_id = "pgxind-kftx5g4t")
```
...
type, output, biosample_id, save_file, filename
...
```{r}
biosamples <- pgxLoader(type="b", filters = "PMID:20229506", limit=2)
...
```{r}
variant_1 <- pgxLoader(type="v", biosample_id = biosample_id)
head(variant_1)
```
...
```{r}
variant_2 <- pgxLoader(type="v", biosample_id = biosample_id,output = "pgxseg")
head(variant_2)
```
...
```{r}
variant_3 <- pgxLoader(type="v", biosample_id = biosample_id,output = "seg")
head(variant_3)
```
...
```{r eval=FALSE}
pgxLoader(type='v', output='pgxseg', biosample_id=biosample_id, save_file=TRUE, 
...
```{r eval=FALSE}
pgxLoader(type='v', output='seg', biosample_id=biosample_id, save_file=TRUE, 
