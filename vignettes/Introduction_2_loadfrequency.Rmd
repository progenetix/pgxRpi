...
freq_pgxseg <- pgxLoader(type="frequency", output ='pgxseg',
                         filters=c("NCIT:C4038","icdom-85003"), 
                         codematches = TRUE)
```
...
filters. The matrix with the name 'total' is the the CNV frequency matrix where matrices from all 
...
You can access the pgxseg matrix of one filter from this list.  
...
The `data` slot contains a list of the nine CNV frequency matrices. One of these eight
...
In the pgxmatrix matrix, `filters` is the same as that in the pgxseg matrix. It's followed by all “gain status” columns (3102 intervals) plus all “loss status” columns (3102 intervals).
...
* `filters`: identifiers of interests such as c("NCIT:C7376","icdom-98353").
...
```
# Visualization of CNV frequency data using 'pgxseg' matrix
...
* `filters`: a string value to indicate which filter to be plotted. The length of filters is limited to one if the parameter `circos` is False. Default is the first filter.
...
pgxFreqplot(freq_pgxseg, filters='icdom-85003')
```
...
pgxFreqplot(freq_pgxseg, filters='icdom-85003', circos = TRUE)
```

The circos plot also supports multiple group comparison

```{r,fig.width=6, fig.height=6}
pgxFreqplot(freq_pgxseg,filters= c("NCIT:C4038","icdom-85003"),circos = TRUE) 
```

# Clustering of CNV frequency data using 'pgxmatrix' matrix
...
* `filters`: A single or a comma-concatenated list of identifiers such as c("NCIT:C7376","icdom-98353")
