...
The ".pgxseg" data format is used within Progenetix to store variant data encompassing CNV (Copy Number Variation) and SNV (Single Nucleotide Variant), along with the metadata of associated samples. This vignette describes how to manipulate local ".pgxseg" files using this package.
...
The returned object is same as the CNV frequency object with "pgxfreq" format returned by `pgxLoader` function (format details see the vignette Introduction_3_loadfrequency. It contains two slots 'meta' and 'data'.
