...
The data contains 44 columns representing different aspects of biosample information
...
```{r}
biosamples[biosamples$histological_diagnosis_id != "NCIT:C3059",'id']
```
...
```{r}
biosample_id <- biosamples$id[c(1:20)]
```
...
(duplication) or 'DEL' (deletion) in specific chromosome locations. The column referenceName is chromosome.
