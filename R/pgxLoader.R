...
#' and the next 500 profiles are returned. Default is `NULL`, meaning no profiles are skipped.
...
pgxLoader <- function(
    ...
    type <- match.arg(type, c("biosamples", "individuals","g_variants","analyses","filtering_terms","cnv_frequency","cnv_fraction","sample_count"))
    ...
    ## non-beacon parameter usage
    if (type %in% c("analyses","filtering_terms","g_variants","cnv_frequency","sample_count")){
    ...
    switch(type,
           ...
           filtering_terms = pgxmetaLoader(type=type,biosample_id=NULL,individual_id=NULL,filters=NULL,codematches=FALSE,skip=NULL,limit=NULL,domain=domain,entry_point=entry_point,dataset=NULL),
