...
#'
#' @param data The meatdata of individuals returned by `pgxLoader` function.
#' @param group_id A string specifying which column is used for grouping in the Kaplan-Meier plot.
#' @param condition Condition for splitting individuals into younger and older groups. Only used if `group_id` is age related.
...
pgxMetaplot <- function(data, group_id, condition, return_data = FALSE,...){
    ...
    meta.sel.df <- data[data$index_disease_followup_time != "None" & data$index_disease_followup_state_id != 'EFO:0030039',]
    if(nrow(meta.sel.df) == 0) stop("\n No available survival data \n")
    
    # transform ISOdate interal to days
    plot_followup_time <- lubridate::time_length(meta.sel.df$index_disease_followup_time,unit='day')
  
    # process group id
    ## for age related group id
    if (group_id %in% c("age_iso","age_days")){
    ...
    plot_followup_state_id <- meta.sel.df$index_disease_followup_state_id
